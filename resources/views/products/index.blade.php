    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        @forelse ($products as $product)
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <img src="{{ $product->image_url ? asset('storage/products/' . $product->image_url) : 'default_image.jpg' }}" alt="{{ $product->name }}" class="w-full h-48 object-cover transition-transform transform hover:scale-105">
                <div class="p-6">
                    <h3 class="text-xl font-semibold text-gray-800">{{ $product->name }}</h3>
                    <p class="text-teal-500 text-lg mt-2">${{ number_format($product->price, 2) }}</p>
                    <p class="text-gray-600 text-sm mt-2">Producto de segunda mano en excelente estado.</p>
                    <div class="flex justify-between items-center mt-4">
                        <a href="{{ route('product.show', $product->id) }}" class="bg-teal-500 text-white px-4 py-2 rounded-md hover:bg-teal-600">Más Info</a>
                    </div>
                </div>
            </div>
        @empty
